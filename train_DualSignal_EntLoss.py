...
def train(net, epoch):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        # --- Config C: Cross-entropy + EntLoss ---
        ce_loss = loss.mean()

        # EntLoss: minimize entropy → model more confident
        # torch.sum(p * log(p)) is negative, adding it subtracts entropy from loss
        ent_loss = torch.sum(
        ...
        training_grad_norm += margin.mean().item()
...
if __name__ == '__main__':
    print('=== Config C: Dual-Signal Magnitude + EntLoss Training ===')
