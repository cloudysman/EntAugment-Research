...
class CIFAR10Dataset(Dataset):
    ...
    def __init__(self, root, train=True, transform=None, target_transform=None, aug=None):
        ...
        # ── NEW: external_transform bypass EntAugment logic entirely ──
    ...
    def set_transform(self, magnitude, cutout_length):
        ...
        for i in range(len(magnitude)):
            transform_list.append(
                self.make_magnitude_transform(magnitude=magnitude, cutout_length=cutout_length)
            )
    ...
    def __getitem__(self, index):
        if self.train:
            ...
            # ── NEW: nếu có external_transform, dùng nó (bypass EntAugment) ──
            if self.external_transform is not None:
                normalized_img = self.external_transform(img)
                return index, normalized_img, target

            # Logic EntAugment giữ nguyên
            if self.is_warmup_finished:
                t = self.make_magnitude_transform(
                    magnitude=self.MAGNITUDE[index].item(), cutout_length=16
                )
...
class CIFAR100Dataset(Dataset):
    def __init__(self, root, train=True, fine_label=True, transform=True, aug=None):
        if train:
            self.data, self.labels, self.filename_list = load_CIFAR_100(
                root, train, fine_label=fine_label
            )
        else:
    ...
    def __getitem__(self, index):
        if self.train:
            ...
            if self.is_warmup_finished:
                t = self.make_magnitude_transform(
                    magnitude=self.MAGNITUDE[index].item(), cutout_length=8
                )
                normalized_img = t(img)
            else:
                normalized_img = self.warmup_transform(img)
