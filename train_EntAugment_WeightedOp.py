...
parser.add_argument('--temperature', type=float, default=1.0,
                    help='Softmax temperature (higher=more uniform, lower=more greedy)')
...
# ─────────────────────────────────────────────
# Weighted Operation Selector (Softmax Random)
# ─────────────────────────────────────────────

class WeightedOperationSelector:
    """
    Softmax weighted random operation selection.

    Unlike UCB (always picks best arm → loses diversity),
    this converts rewards into selection probabilities via softmax.
    Better operations are selected MORE OFTEN but not ALWAYS,
    preserving diversity while exploiting learned knowledge.
    """
    def __init__(self, operations, temperature=1.0):
    ...
    def select(self, epoch=None):
        """
        First K calls: round-robin to try every operation once.
        After that: random choice weighted by softmax of avg reward.
        """
        ...
        shifted = (avg_reward - avg_reward.max()) / self.temperature
    ...
    def stats(self):
        ...
        total = self.counts.sum()
        if total >= self.K:
            shifted = (avg - avg.max()) / self.temperature
            exp_scores = np.exp(shifted)
            probs = exp_scores / exp_scores.sum()
        else:
            probs = np.ones(self.K) / self.K
        return ' | '.join('{}: n={:.0f} r={:.4f} p={:.2f}'.format(
            self.op_names[i], self.counts[i], avg[i], probs[i])
...
def train(net, epoch, selector):
    ...
    epoch_op_counts = np.zeros(selector.K)

    for i, data in enumerate(train_loader, 0):
        idx, inputs, labels = data
        inputs, labels = inputs.cuda(), labels.cuda()

        # Weighted random selection PER-BATCH (non-deterministic!)
        op_idx = selector.select(epoch)
        ...
        if (i + 1) % args.log_interval == 0:
            ...
            top_op = selector.op_names[int(np.argmax(epoch_op_counts))]
...
if __name__ == '__main__':
    print('=== EntAugment + Weighted Operation Selection (per-batch) ===')
    print('Seed: {}, Temperature: {}'.format(args.seed, args.temperature))
    print('Dataset: {}, Model: {}, Epochs: {}'.format(
        args.dataset, cfg['model']['type'], epoches))

    selector = WeightedOperationSelector(ALL_TRANSFORMS, temperature=args.temperature)
    prev_val_loss = compute_val_loss(model)

    for epoch in tqdm(range(start_epoch, epoches)):
        most_used_op = train(model, epoch, selector)

        current_val_loss = compute_val_loss(model)
        reward = prev_val_loss - current_val_loss
        selector.update(most_used_op, reward)
        ...
        if (epoch + 1) % 10 == 0:
            print('\n[Selector Stats @ epoch {}]\n{}\n'.format(epoch, selector.stats()))
    ...
    with open(result_file, 'a', newline='') as f:
        ...
        writer.writerow([args.dataset, cfg['model']['type'], args.seed,
                         best_epoch, best_acc, 'EntAugment_WeightedOp'])
